...
public partial class frmApprovePRI : System.Web.UI.Page
{
    ...
    protected void Search()
    {
        if (txtdate.Text != "")
        {
            DataTable DtVerifyData = Lo.RetriveCodeRawDataGridSimple(0, "PRI-Fixed Line", "", Convert.ToDateTime(txtdate.Text), "N", DateTime.Now, "N", "1", "", "BindGridApproved2nd");
        ...
        try
        {
            ...
            foreach (GridViewRow row in gvexcel.Rows)
            {
                ...
                if ((row.FindControl("chkRow") as CheckBox).Checked)
                {
                    ...
                    {
                        string Uploadexcel = Lo.RetriveCodeLocalup(0, HfID.Value, "", "", "UExcelVerifyStatusPRI");
                        Totalcountsucess++;
                    }
                    else
                    {
        ...
        try
        {
            if (txtreasone.Text != "" && lbltotalrowselected.Text != "")
            {
                foreach (GridViewRow row in gvexcel.Rows)
                {
                    ...
                    if ((row.FindControl("chkRow") as CheckBox).Checked)
                    {
                        int DeleteAllrow = Lo.DeleteCode(0, HfID.Value, Convert.ToDateTime(txtdate.Text).ToString("dd/MMM/yyyy"), "", "DeletecodeAppExclPRI");
                    }
                }
                SendMailTL();
                Search();
