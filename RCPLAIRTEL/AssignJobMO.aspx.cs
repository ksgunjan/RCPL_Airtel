...
public partial class AssignJobMO : System.Web.UI.Page
{
    Logic Lo = new Logic();
    ...
    protected void BindAssignJobCompany()
    {
        ...
        {
            DtFetchcompgropby = Lo.RetriveCodeMOFE(0, "MO", "", DateTime.Now, hfFeName.Value, "", "", "", "", "BindGridMONew");
            ViewState["mData"] = DtFetchcompgropby;
        }
        else
        {
            DataTable dtn = (DataTable)ViewState["mData"];
            DataView mDv = new DataView(dtn);
            mDv.RowFilter = "Prod_Type_Name= '" + ddlprodbycat.SelectedItem.Text + "'";
            DtFetchcompgropby = mDv.ToTable();
        }
    ...
    protected void gvexcel_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            ...
            {
                string lblbilledextid = (gvexcel.FindControl("lblbilledextid") as Label).Text;
                string allocatedDate = (gvexcel.FindControl("hfAllocatedDate") as HiddenField).Value;
                if (lblbilledextid.ToString() == "#")
                    lblbilledextid = "";
                DataTable dtgetid = Lo.RetriveCodeMOFE(0, "", "", DateTime.Now, e.CommandArgument.ToString(), lblbilledextid.ToString(), allocatedDate.ToString(), "", "", "getbilledid");
                hflocsrid.Value = dtgetid.Rows[0]["ExcelRawID"].ToString();
                DataTable DtSearchView = Lo.RetriveCodeMOFE(Convert.ToInt64(hflocsrid.Value), "", "", DateTime.Now, "", "", "", "", "", "ViewDetail");
            ...
            else if (e.CommandName == "UpdateStatus")
            {
                ...
                if (stattusbyfe == "" || stattusbyfe == null || stattusbyfe == "Pending")
                {
                    ...
                    string CustEmail = (gvexcel.FindControl("lblemailcustomer") as Label).Text;
                    hfcustomerorpartyname.Value = (gvexcel.FindControl("lblcoordinatorname") as Label).Text;
                    hfbilledextid1.Value = (gvexcel.FindControl("lblbilledextid") as Label).Text;
                    if (hfbilledextid1.Value == "#")
                        hfbilledextid1.Value = "";
                    hfAllocationDate.Value = (gvexcel.FindControl("hfAllocatedDate") as HiddenField).Value;
                    hfpartyname.Value = e.CommandArgument.ToString();
                    hfEmailCustomer.Value = CustEmail.ToString();
                    ...
                    DataTable dtgetid = Lo.RetriveCodeMOFE(0, "", "", DateTime.Now, e.CommandArgument.ToString(), hfbilledextid1.Value, hfAllocationDate.Value, "", "", "getbilledid");
                    hflocsrid.Value = dtgetid.Rows[0]["ExcelRawID"].ToString();
                    ScriptManager.RegisterClientScriptBlock(this, GetType(), "none", "<script>$('#myModal').modal('show');</script>", false);
                }
                else
            ...
            {
                string lblacdate = (gvexcel.FindControl("hfAllocatedDate") as HiddenField).Value;
                string billextid = (gvexcel.FindControl("lblbilledextid") as Label).Text;
                if (billextid.ToString() == "#")
                    billextid = "";
                DataTable DtGetCompNum = Lo.RetriveCodeMOFE(0, "", "", DateTime.Now, e.CommandArgument.ToString(), lblacdate.ToString(), billextid.ToString(), "", "", "MobNo");
