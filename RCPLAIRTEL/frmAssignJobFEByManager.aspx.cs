...
public partial class frmAssignJobFEByManager : System.Web.UI.Page
{
    ...
    protected void ddlfe1_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            dtgrid = Lo.RetriveAssignJobFEByManager("Leased Line", ddlfe1.SelectedItem.Text.Trim(), "BindGrid");
        ...
        if (ddlfe.SelectedItem.Text != "" && ddlfe1.SelectedItem.Text != "")
        {
            try
            {
                foreach (GridViewRow row in gvexcel.Rows)
                {
                    ...
                    {
                        string UpdateNewFEtoLocation = Lo.UpdateCode(ddlfe.SelectedItem.Text, HfID.Value, "", "", "", "", "", "", "", "", "UpdateLLAJFBM");
                    }
                }
                string message = new System.Web.Script.Serialization.JavaScriptSerializer().Serialize("Successfully FE Assign");
                string script = string.Format("alert({0});window.location ='Manager-Assign-FE-Job';", message);
