...
public partial class frmPositiveWeeklyReport : System.Web.UI.Page
{
    ...
    {
        DataTable DtLL = Lo.RetriveCodeReport9("" + DateTime.Now.AddDays(-7).ToString("yyyy/MMM/dd") + "", "" + DateTime.Now.AddDays(-1).ToString("yyyy/MMM/dd") + "", "LLReport");
    ...
    {
        dt = Lo.RetriveFunctionCode6("'MO'", "" + DateTime.Now.AddDays(-7).ToString("yyyy/MMM/dd") + "", "" + DateTime.Now.AddDays(-1).ToString("yyyy/MMM/dd") + "", "'Positive'", "");
        if (dt.Rows.Count > 0)
        {
            gvmo.DataSource = dt;
            gvmo.DataBind();
    ...
    {
        DataTable DtPRI = Lo.RetriveCodeReport9("" + DateTime.Now.AddDays(-7).ToString("yyyy/MMM/dd") + "", "" + DateTime.Now.AddDays(-1).ToString("yyyy/MMM/dd") + "", "PRIReport");
    ...
    protected void btnmail_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (DtLL.Rows.Count > 0 || dt.Rows.Count > 0 || DtPRI.Rows.Count > 0)
            {
                ...
                DataTable update = Lo.RetriveCodeReport9("" + DateTime.Now.ToString("yyyy/MMM/dd") + "","", "Insert");
    ...
    protected void SendMailExcelReport(byte[] bytes, byte[] Wali, byte[] PRI, DataTable DtNotEligible, DataTable dt, DataTable dtPRI, string FileName)
    {
        ...
        {
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), "alert", "ErrorMssgPopup('Report send to Shyam Prasad successfully.')", true);
        }
        else
        {
