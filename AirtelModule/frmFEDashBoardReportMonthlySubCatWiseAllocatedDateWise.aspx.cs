...
public partial class frmFEDashBoardReportMonthlySubCatWiseAllocatedDateWise : System.Web.UI.Page
{
    ...
    {
        DTSearch = Lo.RetriveCodeWithContidion("select [Month],[Not Eligible],[Positive],[Negative],[Pending],[Revisit] from (select Datename(Month,[Allocated Date]) as [Month] ,(case when StatusByFE is null then 'Pending' when StatusByFE='' then 'Pending' when StatusByFE='Pending' then 'Revisit' else StatusByFE end) as statusbyfe from tbl_trn_rawdata where ProdTypeName='MO-Quarterly' group by DateName(Month,[Allocated Date]),[party name],statusbyfe) Tab1 pivot(count(StatusByFE) for StatusByFE in ([Positive],[Negative],[Pending],[Not Eligible],[Revisit])) as Tab2 order by [Month]");
        if (DTSearch.Rows.Count > 0)
        {
    ...
    protected void btnreport_Click(object sender, EventArgs e)
    {
        try
        {
            DataTable dtll = Lo.RetriveCodeWithContidion("select [Month],[Not Eligible],[Positive],[Negative],[Pending],[Revisit] from (select Datename(Month,[Allocated Date]) as [Month],(case when StatusByFE is null then 'Pending'  when StatusByFE='' then 'Pending' when StatusByFE='Pending' then 'Revisit' else StatusByFE end) as statusbyfe from tbl_trn_rawdata where ProdTypeName='Leased Line-Initial')Tab1 pivot(count(StatusByFE) for StatusByFE in ([Positive],[Negative],[Pending],[Not Eligible],[Revisit])) as Tab2");
            ...
            DataTable dtpri = Lo.RetriveCodeWithContidion("select [Month],[Not Eligible],[Positive],[Negative],[Pending],[Revisit] from (select Datename(Month,[Allocated Date]) as [Month],(case when StatusByFE is null then 'Pending'  when StatusByFE='' then 'Pending' when StatusByFE='Pending' then 'Revisit' else StatusByFE end) as statusbyfe from tbl_trn_rawdata where ProdTypeName='PRI-Fixed Line' and KeyDup='false')Tab1 pivot(count(StatusByFE) for StatusByFE in ([Positive],[Negative],[Pending],[Not Eligible],[Revisit])) as Tab2");
