...
public partial class frmApprovePRI : System.Web.UI.Page
{
    Logic Lo = new Logic();
    Label lblproduct;
    Label feName;
    ...
    protected void Search()
    {
        if (txtdate.Text != "")
        {
            DataTable DtVerifyData = Lo.RetriveCodeAllExcelRecordWhereCondition("IsActive='N' and IsClosed='N' and [Allocated Date]='" + txtdate.Text + "' and ExcelVerifiy='1' and ProductName ='PRI-Fixed Line'");
        ...
        try
        {
            ...
            foreach (GridViewRow row in gvexcel.Rows)
            {
                ...
                if ((row.FindControl("chkRow") as CheckBox).Checked)
                {
                    ...
                    {
                        if (lblproduct.Text == "PRI-Fixed Line")
                        {
                            int Uploadexcel = Lo.UpdateCodeOther("tbl_trn_RawData", "Set ExcelVerifiy='2'", "[Subs Del No]='" + HfID.Value + "'");
                        }
                        Totalcountsucess++;
                    }
                    else
            ...
            if (txtreasone.Text != "" && lbltotalrowselected.Text != "")
            {
                foreach (GridViewRow row in gvexcel.Rows)
                {
                    ...
                    if ((row.FindControl("chkRow") as CheckBox).Checked)
                    {
                        if (lblproduct.Text == "PRI-Fixed Line")
                        {
                            Int32 DeleteAllrow = Lo.DeleteCode("tbl_trn_RawData", "[Subs Del No]='" + HfID.Value + "' and  [Allocated Date]='" + Convert.ToDateTime(txtdate.Text).ToString("dd/MMM/yyyy") + "'");
                        }
                    }
                }
                SendMailTL();
