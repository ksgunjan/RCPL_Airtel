...
public partial class frmAssignJobFEByManagerPRI : System.Web.UI.Page
{
    ...
    protected void ddlfe1_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            dtgrid = Lo.RetriveCodeAllExcelRecordWhereCondition("IsClosed='N' and (StatusByFE='Pending' or StatusByFE is null or StatusByFE='') and [Eligible/Not Eligible]='Eligible' and ProductName='PRI-Fixed Line' and KeyDUp='false' and [FE Name]='" + ddlfe1.SelectedItem.Text.Trim() + "'");
        ...
        if (ddlfe.SelectedItem.Text != "" && ddlfe1.SelectedItem.Text != "")
        {
            try
            {
                foreach (GridViewRow row in gvexcel.Rows)
                {
                    ...
                    {
                        int UpdateNewFEtoLocation = Lo.UpdateCodeOther("tbl_trn_RawData", "Set [FE Name]='" + ddlfe.SelectedItem.Text + "'", "[Subs Del No]='" + HfID.Value + "' and ProductName='PRI-Fixed Line'");
                    }
                }
                ...
                var script = string.Format("alert({0});window.location ='AssignJob-FeByManager-PRI';", message);
